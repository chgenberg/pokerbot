...
MAX_HISTORY = 8  # Hur många utbyten (fråga+svar) som max skickas till GPT
...
def gpt_is_solver_question(question: str) -> bool:
    """Use GPT to determine if the question is a hand history/solver spot."""
    prompt = (
        "Decide if this question is a hand history or a situation where the user wants to know what the solver would do (e.g. 'what do you do with AKs on CO after fold, fold?'). "
        "Reply only with 'YES' if it is such a question, otherwise 'NO'.\n\nQuestion: " + question
    )
    try:
        rsp = openai.ChatCompletion.create(
            model="gpt-4o-mini",
            messages=[
                {"role": "system", "content": "You are a poker coach. Reply only with YES or NO."},
                {"role": "user",   "content": prompt},
            ],
            max_tokens=2,
            temperature=0.0,
        )
        content = rsp.choices[0].message["content"].strip().upper()
...
def main() -> None:
    ...
    chat_history = []  # Spara (roll, text) för hela konversationen
    ...
    while True:
        ...
        # --- NEW: Decide if this is a solver question or not ---
        if not gpt_is_solver_question(q):
            # Bygg upp historik för coach
            messages = [{"role": "system", "content": "You are a pedagogical poker coach."}]
            for role, text in chat_history[-MAX_HISTORY*2:]:
        ...
        # --- Otherwise: normal solver pipeline ---
        ...
        print("GPT parse result:", parsed)  # DEBUG
        if parsed:
        ...
        log(q, final_ans)
        chat_history.append(("user", q))
        chat_history.append(("assistant", final_ans))
