...
def main() -> None:
    ...
    while True:
        ...
        if not gpt_is_solver_question(q):
            # Answer as a poker coach instead
            coach_prompt = (
                "You are a pedagogical poker coach. Answer the question below concisely and clearly, without using solver data or charts.\n\nQuestion: " + q
            )
            try:
                rsp = openai.ChatCompletion.create(
                    model="gpt-4o-mini",
                    messages=[
                        {"role": "system", "content": "You are a pedagogical poker coach."},
